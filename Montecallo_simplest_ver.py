...
# drive.mount('/content/drive') # Excelを使用する場合はコメントアウトを外す
                ...
                for attempt in range(attempts):
                    ...
                    while True:
                        ...
                        if all_complete:
                            ...
                            Fiderity_count = sum(seg.storage_count for seg in segments)+num_len+1
                            ...
                            # ---------------------------------------------------
                            # 2. 【追加】通信待ち時間 (Communication Latency)
                            # ---------------------------------------------------
                            # 光ファイバー中の光速 (約20万km/s)
                            #c_fiber = 200000.0

                            # 通信距離 (km) = セグメント数(N) × 1セグメントのEL数(n) × ELの長さ(l)
                            #total_distance = sim_params["num_segments"] * sim_params["n_ELs"]*param_dict.get("l")

                            # 1ステップあたりの往復時間 (RTT) = 距離 / 光速
                            # ※ Method Aでは、結果を知るために必ずこの時間がかかります
                            #t_rtt = total_distance / c_fiber

                            # 総待ち時間 = ステップ数 × 1回の往復時間
                            #t_latency_total = step * t_rtt

                            # ---------------------------------------------------
                            # 3. 合計時間 (Total Time)
                            # ---------------------------------------------------
                            # 「光を出した時間」 + 「結果を待った時間」
                            #t_elapsed = t_generation + t_latency_total

                            # デバッグ表示（確認用）
                            # print(f"Gen: {t_generation:.6f}, Latency: {t_latency_total:.6f}, Dist: {total_distance}km")
                            ...
                            # 成功したら記録して次の試行へ
                            #t_elapsed = step * param_dict.get("t_AFC")*10
                            #if method_choice == "A":
                              #t_elapsed = step * param_dict.get("t_AFC")
                              #print(param_dict.get("t_AFC"))
                            #else:
                              #t_elapsed = (step * param_dict.get("t_AFC"))/param_dict.get("separate")
...
# 分母の log の中身: 1 - (eta_qst_total^2 * ...)
          ...
          while True :
            ...
            if Sndmethod_choice == "B":
              print(f"{num+1}の時のトータルフィデリティーの平均値{F_list[num]}")
            else:
