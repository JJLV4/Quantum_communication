...
class LQUOMExpectedTimeParams:
    ...
    n: int = 1      
    N_temp:int =1    # number of ELs inside one ARC (blue-box reproduction uses n=1)
    t_spdc:float =1e-6        #t_EPPS
    # distance
...
# ------------------------------------------------------------
def expected_trials_until_all_links_ready_appendix_d(num_links: int, p_link: float,params:LQUOMExpectedTimeParams) -> float:
    """
    ...
    if p_link >= 1.0:
        return 1.0
    #nattempts=9091
    p_EL1=1-(1-p_link)**params.N_temp
    #====95%の信頼度をつかったとき、求めるτ======
    """
    omega_epps=10000
    epsilon=0.05
    t_gen=params.t_QST + params.t_AFC + params.t_CNOT
    numerator=np.log(1.0-(1.0- epsilon)**(1.0/params.N))
    denominator=omega_epps * np.log(1.0-p_link)
    tau=numerator/denominator +t_gen
    n_attempt=omega_epps*(tau-t_gen)
    p_1p=(1-(1-p_link)**n_attempt)
    """
    #=========================================
    s = 0.0
    for j in range(1, num_links + 1):
        denom = 1.0 - (1.0 - p_EL1) ** j 
        if abs(denom) < 1e-15:
            return np.inf
...
def t_round_arc(params: LQUOMExpectedTimeParams) -> float:
    ...
    """
    return params.N_temp*params.t_QST + params.t_AFC+params.N_temp *params.t_spdc
...
def expected_time_all_arcs_ready(params: LQUOMExpectedTimeParams) -> float:
    ...
    num_links = params.N
    return t_round_arc(params) * expected_trials_until_all_links_ready_appendix_d(num_links, p_link,params)
